...
import type { SegmentData } from '@/data/market-data'
...
export function SegmentChart({ data }: SegmentChartProps) {
  const filtered = data.filter((d) => d.apartments > 0 || d.villas > 0)
  ...
  return (
    <ResponsiveContainer width="100%" height={280}>
      <BarChart data={filtered} margin={{ top: 8, right: 8, left: 8, bottom: 0 }}>
        ...
        <Bar dataKey="apartments" name="Appartements" fill="#2E86AB" radius={[4, 4, 0, 0]} />
        <Bar dataKey="villas" name="Villas" fill="#C9A96E" radius={[4, 4, 0, 0]} />
      </BarChart>
    </ResponsiveContainer>
  )
}
