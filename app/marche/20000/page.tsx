...
import { MARKET_20000 } from '@/data/market-data'

export const revalidate = 2592000

export const metadata: Metadata = {
  title: 'Prix immobilier Ajaccio 20000 — DVF Centre-ville, Cours Napoléon',
  description:
    "Prix au m², tendances DVF et transactions récentes à Ajaccio 20000 : centre-ville, cours Napoléon, quartier des Étrangers, Trottel, Parc Berthault, Terre Sacrée, route des Sanguinaires. Données actualisées au 1er mai 2026.",
  openGraph: {
    title: 'Prix immobilier Ajaccio 20000 — DVF Centre-ville',
    description:
      "Prix médian 3 400 €/m² pour un appartement, 4 200 €/m² pour une villa. Données DVF officielles.",
    url: 'https://ajaccio-estimation.fr/marche/20000',
    type: 'website',
  },
}
...
const breadcrumbSchema = {
  ...
  itemListElement: [
    ...
    { '@type': 'ListItem', position: 3, name: 'Ajaccio 20000', item: 'https://ajaccio-estimation.fr/marche/20000' },
  ],
}
...
const datasetSchema = {
  '@context': 'https://schema.org',
  '@type': 'Dataset',
  name: 'Prix immobiliers Ajaccio 20000 — DVF',
  description:
    'Données DVF agrégées pour le code postal 20000 (Ajaccio centre)',
  url: 'https://ajaccio-estimation.fr/marche/20000',
...
const URBANISME_ITEMS = [
  {
    ...
    content:
      "Zone mixte UA/UB prédominante en centre-ville. Hauteurs limitées à R+4 dans le périmètre historique.",
  },
  {
    title: "PPRI — Plan de Prévention des Risques d'Inondation",
    content:
      "Plan de Prévention des Risques d'Inondation approuvé. Consultez le portail de la mairie d'Ajaccio.",
  },
  {
    title: 'Projets urbains',
    content:
      "Réhabilitation du cours Napoléon (2024-2026). Extension du réseau Kallisté Bus.",
  },
]

const { monthlyPrices, segments, recentTransactions } = MARKET_20000

export default function Marche20000Page() {
  return (
    <main className="max-w-5xl mx-auto px-4 pt-32 pb-10">
      ...
      <nav aria-label="Fil d'Ariane" className="text-sm text-[#9B9B9B] mb-6 flex items-center gap-2">
        ...
        <span>/</span>
        <span className="text-[#1B4F72] font-medium" aria-current="page">Ajaccio 20000</span>
      </nav>

      {/* Section 1 — H1 + AEO */}
      <div className="mb-8">
        <h1 className="font-[family-name:var(--font-poppins)] text-3xl font-bold text-[#1B4F72] mb-2">
          Prix immobilier Ajaccio 20000
        </h1>
        <DataFreshnessBadge />
        <p className="mt-4 text-[#5C5C5C] leading-relaxed max-w-3xl">
          À Ajaccio (20000), secteur comprenant le centre-ville, le cours Napoléon, le quartier des
          Étrangers, Trottel, le Parc Berthault, la Terre Sacrée et la route des Sanguinaires, le
          prix médian au m² est de{' '}
          <strong className="text-[#1B4F72]">3 400 €</strong> pour un appartement et{' '}
          <strong className="text-[#1B4F72]">4 200 €</strong> pour une villa, selon les données DVF
          actualisées au{' '}
